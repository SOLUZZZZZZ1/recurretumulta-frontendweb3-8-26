...
class RtmPresenterFrontendContractTest(unittest.TestCase):
    ...
    def test_signer_station_is_a_separate_closed_local_surface(self):
        ...
        self.assertIn('operator?.roleCode === REQUIRED_ROLE', page)
        ...
        self.assertIn("loginOpsOperator", page)
        self.assertIn("readOpsAuthStatus", page)
        self.assertIn("logoutOpsOperator", page)
        self.assertNotIn("readAuthJson", page)
        self.assertIn("bindOpsSessionLifecycle(window", page)
        self.assertIn("scheduleOpsSessionExpiry", page)
        self.assertIn('sensitiveRootRef.current?.setAttribute("hidden", "")', page)
        self.assertIn("hidden={!viewVisible}", page)
        self.assertIn("sessionAbortRef.current?.abort()", page)
        self.assertIn("signal: sessionAbortRef.current?.signal || null", page)
